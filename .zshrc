# set Z Shell (zsh) as default shell if in codespace
...
PROMPT="%F{219}%B%S aszm101 %s%b %1~ %# %f" # left: pink foreground / text color, bold, standout mode (swaps foreground + background), current folder, automatic prompt (% for normal user in zsh, # for root / admin user in both zsh and bash)
